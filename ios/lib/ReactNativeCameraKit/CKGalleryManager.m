...
-(PHFetchResult *)topLevelUserCollections {
    if (!_topLevelUserCollections) {
        _topLevelUserCollections = [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeAlbum
...
-(void)extractCollectionsDetails:(PHFetchResult*)collections
    ...
    [self.topLevelUserCollections enumerateObjectsUsingBlock:^(PHAssetCollection *collection, NSUInteger idx, BOOL *stop) {
        [localIDList addObject:collection.localIdentifier];
    }];
    [self.smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection *collection, NSUInteger idx, BOOL *stop) {
        [localIDList addObject:collection.localIdentifier];
    }];
    ...
    NSInteger collectionCount = allAlbums.count;
    if (collectionCount == 0) {
        if (block) {
            block(nil);
        }
...
RCT_EXPORT_METHOD(getAlbumsWithThumbnails:(RCTPromiseResolveBlock)resolve
    ...
    [self extractCollectionsDetails:self.smartAlbums
